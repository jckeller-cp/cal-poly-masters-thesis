...
using the Svelte JavaScript framework that allows developers and players to
...
\begin{itemize}
    ...
    \item The author was familiar with both Rust and Svelte.
\end{itemize}
...
This split script design allows for the separation of the rules that guide characters
...
story with all manners of interaction as is provided with Inform 7. \cite{inform_7_inform_nodate}
...
which edge of a directional relation it is on. An example world state can be found in \Cref{fig:WorldStateGraph}.
...
relations, which allows for a simulation layer to be built on top of it. For example,
...
Simulation functions that modify the world, such as most of the practice outcomes\slash effects,
...
each action. This is functional solution, but it does
...
in the world state and provides the ability to roll the world state back through a transactional
...
decays when it is passed down the recursion chain. In this case, the score delta from
...
the planner to prioritize quick positive results over those that may take several turns.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Even though both chains of actions end up in the same spot, the one on the left "decays" due to its longer length.}
